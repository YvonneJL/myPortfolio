...
export default function SingleSkill({ path, name, gridPos, index }: ISingleSkillProps) {
  ...
  return (
    <div
      ref={ref}
      className={`
        ${gridPos} relative lg:pl-5 pl-2 flex items-center lg:w-30 lg:h-20 w-20 h-15
    ...
    >
      ...
      />
      <p className="absolute lg:text-sm text-[10px] opacity-0 group-hover:opacity-100 transition-opacity duration-300 ease-in-out">
        {name}
      </p>
    </div>
  );
